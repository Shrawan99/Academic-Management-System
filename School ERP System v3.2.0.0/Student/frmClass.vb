...
Public Class frmClass
    ...
    Private Sub DeleteRecord()
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl3 As String = "select Classname from Class,Subject where Class.Classname=Subject.Class and Classname=@d1"
            cmd = New SqlCommand(cl3)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", txtClass.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read Then
                MessageBox.Show("Unable to delete..Already in use in Subject Entry", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                If Not rdr Is Nothing Then
                    rdr.Close()
                End If
                Exit Sub
    ...
    Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb5 As String = "update Subject set Class=@d1 where Class=@d2"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb1 As String = "update BusFeePayment_Student set Student_Class=@d1 where Student_Class=@d2"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb2 As String = "update CourseFeePayment set Student_Class=@d1 where Student_Class=@d2"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb3 As String = "update HostelFeePayment set Student_Class=@d1 where Student_Class=@d2"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb4 As String = "update MarksEntry set Student_Class=@d1 where Student_Class=@d2"
