...
Public Class frmSchoolInfo
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into SchoolInfo( S_Id, SchoolName, Address, ContactNo, AltContactNo, FaxNo, Email, Website,RegistrationNo, EstablishedYear, SchoolType,Logo) VALUES (@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8,@d9,@d12,@d14,@d15)"
            ...
            cmd.ExecuteNonQuery()
            con.Close()
            Dim st As String = "added the new school '" & txtSchoolName.Text & "' info"
    ...
    Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "Update SchoolInfo set SchoolName=@d2, Address=@d3, ContactNo=@d4, AltContactNo=@d5, FaxNo=@d6, Email=@d7, Website=@d8,RegistrationNo=@d9, EstablishedYear=@d12, SchoolType=@d14,Logo=@d15 where S_ID=@d1"
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", txtID.Text)
            cmd.Parameters.AddWithValue("@d2", txtSchoolName.Text)
    ...
    Public Sub Getdata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(S_Id), RTRIM(SchoolName), RTRIM(Address), RTRIM(ContactNo), RTRIM(AltContactNo), RTRIM(FaxNo), RTRIM(Email), RTRIM(Website),RTRIM(RegistrationNo), RTRIM(EstablishedYear), RTRIM(SchoolType),Logo from SchoolInfo order by schoolname", con)
    ...
    Private Sub dgw_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dgw.MouseClick
        Try
            If dgw.Rows.Count > 0 Then
                ...
                txtEstablishedYear.Text = dr.Cells(9).Value.ToString()
                cmbSchoolType.Text = dr.Cells(10).Value.ToString()
    ...
    Private Sub DeleteRecord()
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim clX As String = "select S_ID from SchoolInfo,Installment_Hostel where SchoolInfo.S_ID=Installment_Hostel.SchoolID and S_ID=@d1"
            cmd = New SqlCommand(clX)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", txtID.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read Then
                MessageBox.Show("Unable to delete..Already in use in Installments[Hostel]", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl3 As String = "select S_ID from SchoolInfo,Exam where SchoolInfo.S_ID=Exam.SchoolID and S_ID=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl4 As String = "select S_ID from SchoolInfo,CourseFee where SchoolInfo.S_ID=CourseFee.SchoolID and S_ID=@d1"
